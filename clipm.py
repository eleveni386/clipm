...
class manager():
        ...
        if email:
            ...
            try:
                self.cur.execute(
                        ...
                        (%s,'%s','%s','%s','%s','%s');"%\
                        ("null",\
                        unicode(keys.get("title")),\
                        unicode(keys.get("username")),\
                        unicode(pwd),\
                        time.strftime("%Y-%m-%d",time.localtime()),\
                        unicode(keys.get("remark"))))
                self.sql.commit()
            except sqlite3.Error, e :
                if self.sql:self.sql.rollback()
                print "Error: %s"%e.args[0]
    ...
    def Importkey(self,keyfile):
        try:
            fp = open(keyfile,'r').read()
        except IOError:
            print "not found keyfile in %s"%fp
            sys.exit(1)
...
def Help():
    ...
    listkey 当前密钥列表, 无参数要求

    importkey 导入现有密钥, 要求密钥路径
